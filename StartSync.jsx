...
function AuthPage({ type, onNavigate, onLogin }) {
  ...
  const handle = () => {
    ...
    setError("");
    onLogin({ name: form.name || "Alex Johnson", role, email: form.email });
  };
...
function CollabDashboard({ user, onNavigate }) {
  const stats = [
              ...
              <Card key={s.id} hover style={{ padding: "12px 14px" }} onClick={() => onNavigate("browse")}>
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: C.textMuted }}>{s.roles[0]}</div>
                  </div>
...
// ─── Browse Page ──────────────────────────────────────────────────────────────
function BrowsePage({ onApply, savedIds, onToggleSave, onMessage }) {
  ...
  const handleApply = () => {
    ...
    setAppliedIds([...appliedIds, applyTarget.id]);
    setApplyOpen(false);
    setCoverMsg("");
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
        {filtered.map(s => (
          <Card key={s.id} hover onClick={() => setSelectedStartup(s)}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              ...
              <span onClick={e => { e.stopPropagation(); onToggleSave(s.id); }}
                style={{ fontSize: 18, cursor: "pointer", opacity: savedIds.includes(s.id) ? 1 : 0.4 }}>🔖</span>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 12, color: C.textMuted }}>{s.applications} applied</div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              <div style={{ background: C.bgCard2, borderRadius: 10, padding: "12px 14px" }}>
                ...
                <div style={{ fontSize: 14, color: C.text }}>{selectedStartup.location}</div>
              </div>
              <div style={{ background: C.bgCard2, borderRadius: 10, padding: "12px 14px" }}>
                <div style={{ fontSize: 12, color: C.textMuted, marginBottom: 4 }}>📅 Deadline</div>
                <div style={{ fontSize: 14, color: C.text }}>{selectedStartup.deadline}</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
              <Avatar initials={selectedStartup.founder.avatar} size={34} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{selectedStartup.founder.name}</div>
                <div style={{ fontSize: 12, color: C.textMuted }}>Founder</div>
              </div>
              <Button variant="secondary" size="sm" onClick={() => { setSelectedStartup(null); onMessage(selectedStartup.founder.name); }}>💬 Message</Button>
              {appliedIds.includes(selectedStartup.id)
...
// ─── Founder Dashboard ────────────────────────────────────────────────────────
function FounderDashboard({ user, onNavigate }) {
  const stats = [
    { label: "Startups Posted", value: 2, icon: "🚀", color: C.accent },
    { label: "Total Applications", value: 65, icon: "📋", color: C.warning },
    ...
    <div>
        ...
        {STARTUPS.slice(0, 2).map(s => (
          <Card key={s.id}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[["📋", "Applications", s.applications], ["👥", "Roles", s.roles.length], ["💰", "Comp", s.compensation], ["📍", "Type", s.workType]].map(([ic, lb, vl]) => (
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Post New Startup" width={600}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 16 }}>
          ...
          <Button onClick={() => setCreateOpen(false)}>Post Startup →</Button>
        </div>
      </Modal>
    </div>
  );
...
export default function StartSync() {
  ...
  const renderPage = () => {
    switch (page) {
      case "collab-dashboard": return <CollabDashboard user={user} onNavigate={navigate} />;
      case "browse": return <BrowsePage savedIds={savedIds} onToggleSave={toggleSave} onMessage={() => navigate("messages")} />;
      case "collab-applications": return <ApplicationsPage />;
      ...
      case "founder-dashboard": return <FounderDashboard user={user} onNavigate={navigate} />;
      case "founder-startups": return <FounderStartupsPage />;
      case "founder-applications": return <ApplicationsPage />;
      case "founder-team": return <TeamPage />;
      case "admin-dashboard": return <AdminDashboard />;
      case "admin-users": return <AdminDashboard />;
      case "admin-startups": return <AdminDashboard />;
      default: return <CollabDashboard user={user} onNavigate={navigate} />;
    }
  };
